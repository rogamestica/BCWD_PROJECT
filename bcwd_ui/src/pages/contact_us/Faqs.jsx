      ...
      <Container>
        ...
        />
            ...
            <div style={{ maxWidth: 600, margin: '0 auto' }}>
              <div style={{ backgroundColor: '#4B7CEE', color: '#fff', textAlign: 'center', padding: '10px ', width:'100%' }}>
                <h2>Frequently Asked Questions</h2>
              </div>
            
              {questions.map((question, index) => (
                <div key={index} style={{ border: '2px solid gray', padding: 10, marginBottom: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', width:'100%' }} onClick={() => handleSelect(index)}>
                    <div style={{ marginRight: 10 }}>{selected === index ? '-' : '+'}</div>
                    <div>{question}</div>
                  </div>
